...
from .factories import (
    ...
    TextFragmentGroupFactory, BudgetAccountMarkupFactory,
    BudgetSubAccountMarkupFactory)
...
@pytest.fixture
def create_budget_account_markup(user, db):
    """
    A fixture that creates a :obj:`BudgetAccountMarkup` instance using the
    :obj:`BudgetAccountMarkupFactory`.  Any data that is not explicitly
    provided will be randomly generated by the factory.

    Usage:
    -----
    >>> def test_markup(create_budget_account_markup):
    >>>     markup = create_budget_account_markup(identifier='Test Markup')
    >>>     assert markup.identifier == 'Test Markup'
    """
    def inner(*args, **kwargs):
        kwargs.setdefault('created_by', user)
        kwargs.setdefault('updated_by', user)
        return BudgetAccountMarkupFactory(*args, **kwargs)
    return inner


@pytest.fixture
def create_budget_subaccount_markup(user, db):
    """
    A fixture that creates a :obj:`BudgetSubAccountMarkup` instance using the
    :obj:`BudgetSubAccountMarkupFactory`.  Any data that is not explicitly
    provided will be randomly generated by the factory.

    Usage:
    -----
    >>> def test_markup(create_budget_subaccount_markup):
    >>>     markup = create_budget_subaccount_markup(identifier='Test Markup')
    >>>     assert markup.identifier == 'Test Markup'
    """
    def inner(*args, **kwargs):
        kwargs.setdefault('created_by', user)
        kwargs.setdefault('updated_by', user)
        return BudgetSubAccountMarkupFactory(*args, **kwargs)
    return inner
