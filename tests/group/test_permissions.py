...
def establish_case(another_user_case, multiple_case, collaborator_case,
        logged_in_case, not_logged_in_case, public_case, another_public_case):
...
def update_test_case(api_client, establish_case):
    def inner(domain, data, case):
        group = establish_case(domain, case)
        return api_client.patch("/v1/groups/%s/" % group.pk, data=data)
    return inner
...
def delete_test_case(api_client, establish_case):
    def inner(domain, case):
        group = establish_case(domain, case)
        return api_client.delete("/v1/groups/%s/" % group.pk)
    return inner
...
def test_budget_group_detail_read_permissions(case, assertions,
...
def test_template_group_detail_read_permissions(case, assertions,
        detail_test_case, make_permission_assertions):
...
@pytest.mark.parametrize('case,assertions', GROUP_DELETE_PERMISSIONS + [
    (('multiple_budgets', {'login': True}), {
        'status': 403,
        'error': {
            'message': (
...
def test_budget_group_delete_permissions(case, assertions, delete_test_case,
...
def test_template_group_delete_permissions(case, assertions, delete_test_case,
        make_permission_assertions):
...
def test_budget_group_update_permissions(case, assertions, update_test_case,
        make_permission_assertions):
    response = update_test_case("budget", {'name': 'Test Group'}, case)
    make_permission_assertions(response, case, assertions, path="/")
...
def test_template_group_update_permissions(case, assertions, update_test_case,
        make_permission_assertions):
    response = update_test_case("template", {'name': 'Test Group'}, case)
    make_permission_assertions(response, case, assertions, path="/")
