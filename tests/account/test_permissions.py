...
ACCOUNT_DETAIL_UPDATE_PERMISSIONS = [
    ParameterizedCase(name='another_user', status=403, error={
        'message': (
            'The user must does not have permission to view this account.'),
        'code': 'permission_error',
...
@ParameterizedCase.parameterize(ACCOUNT_DETAIL_UPDATE_PERMISSIONS + [
    ParameterizedCase('another_public_case', status=401),
    ParameterizedCase.collaborator(view_only=403, status=200),
...
def test_budget_account_bulk_permissions(case, detail_update_response, path,
        data):
...
def test_template_account_bulk_permissions(case, detail_update_response, path,
        data):
